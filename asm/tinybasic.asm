...
*
START	BRA.W	CSTART		Cold Start entry point
*GOWARM	BRA.W	WSTART		Warm Start entry point
*OUTCH	BRA.W	OUTC		Jump to character-out routine
*INCH	BRA.W	INC		Jump to character-in routine
GOAUXO	BRA.W	AUXOUT		Jump to auxiliary-out routine
GOAUXI	BRA.W	AUXIN		Jump to auxiliary-in routine
GOBYE	BRA.W	BYEBYE		Jump to monitor, DOS, etc.
*

* Modifiable system constants:
*
TXTBGN	DC.L	TXT		beginning of program memory
ENDMEM	DC.L	$3f000		end of available memory
*
...
ST4	MOVE.L	A4,D0		calculate the length of new line
	...
	BSR.W	MVDOWN		move things out of the way
...
DIRECT	LEA	TAB1,A1
	LEA	TAB1.1,A2
EXEC	BSR.W	IGNBLK		ignore leading blanks
...
RUNNXL	TST.L	CURRNT		executing a program?
	BEQ.W	WSTART		if not, we've finished a direct stat.
...
RUNSML	BSR.W	CHKIO		see if a control-C was pressed
	LEA	TAB2,A1 	find command in TAB2
	LEA	TAB2.1,A2
	BRA	EXEC		and execute it

GOTO	BSR.W	EXPR		evaluate the following expression
	BSR.W	ENDCHK		must find end of line
	MOVE.L	D0,D1
	BSR.W	FNDLN		find the target line
	BNE.W	QHOW		no such line no.
	BRA	RUNTSL		go do it
...
* the number of spaces the value of an expression is INCHg to
...
LS1	BCS	WSTART		warm start if we passed the end
	BSR.W	PRTLN		print the line
	BSR.W	CHKIO		check for listing halt request
	BEQ	LS3
	CMP.B	#CTRLS,D0	pause the listing?
	BNE	LS3
LS2	BSR.W	CHKIO		if so, wait for another keypress
	BEQ	LS2
LS3	BSR.W	FNDLNP		find the next line
	BRA	LS1

PRINT	MOVE	#11,D4		D4 = number of print spaces
	BSR.W	TSTC		if null list and ":"
	DC.B	':',PR2-*
	BSR.W	CRLF		give CR-LF and continue
	BRA	RUNSML		execution on the same line
PR2	BSR.W	TSTC		if null list and <CR>
	DC.B	CR,PR0-*
	BSR.W	CRLF		also give CR-LF and
	BRA	RUNNXL		execute the next line
PR0	BSR.W	TSTC		else is it a format?
	DC.B	'#',PR1-*
	BSR.W	EXPR		yes, evaluate expression
	MOVE	D0,D4		and save it as print width
	BRA	PR3		look for more to print
PR1	BSR.W	TSTC		is character expression? (MRL)
	DC.B	'$',PR4-*
	BSR.W	EXPR		yep. Evaluate expression (MRL)
	BSR	OUTCH		print low byte (MRL)
	BRA	PR3		look for more. (MRL)
PR4	BSR.W	QTSTG		is it a string?
	BRA.S	PR8		if not, must be an expression
PR3	BSR.W	TSTC		if ",", go find next
	DC.B	',',PR6-*
	BSR.W	FIN		in the list.
	BRA	PR0
PR6	BSR.W	CRLF		list ends here
	BRA	FINISH
PR8	MOVE	D4,-(SP)	save the width value
	BSR.W	EXPR		evaluate the expression
	MOVE	(SP)+,D4	restore the width
	MOVE.L	D0,D1
	BSR.W	PRTNUM		print its value
	BRA	PR3		more to print?

FINISH	BSR.W	FIN		Check end of command
	BRA.W	QWHAT		print "What?" if wrong
...
*
GOSUB	BSR.W	PUSHA		save the current 'FOR' parameters
	BSR.W	EXPR		get line number
	MOVE.L	A0,-(SP)	save text pointer
	MOVE.L	D0,D1
	BSR.W	FNDLN		find the target line
	BNE.W	AHOW		if not there, say "How?"
...
RETURN	BSR.W	ENDCHK		there should be just a <CR>
	MOVE.L	STKGOS,D1	get old stack pointer
	BEQ.W	QWHAT		if zero, it doesn't exist
	...
	BSR.W	POPA		and the old 'FOR' parameters
...
* execution continues.
*
FOR	BSR.W	PUSHA		save the old 'FOR' save area
	BSR.W	SETVAL		set the control variable
	MOVE.L	A6,LOPVAR	save its address
	LEA	TAB5,A1 	use 'EXEC' to test for 'TO'
	LEA	TAB5.1,A2
	BRA	EXEC
FR1	BSR.W	EXPR		evaluate the limit
	MOVE.L	D0,LOPLMT	save that
	LEA	TAB6,A1 	use 'EXEC' to look for the
	LEA	TAB6.1,A2	word 'STEP'
	BRA	EXEC
FR2	BSR.W	EXPR		found it, get the step value
	BRA	FR4
...
NX1	CMP.L	D0,D1		test against limit
	...
	BRA	FINISH
NX2	BSR.W	POPA		purge this loop
	BRA	FINISH
...
LIF2	MOVE.L	A0,A1
	...
	BRA.W	WSTART		if no next line, do a warm start
...
INPUT	MOVE.L	A0,-(SP)	save in case of error
	BSR.W	QTSTG		is next item a string?
	BRA.S	IP2		nope
	BSR.W	TSTV		yes, but is it followed by a variable?
...
IP2	MOVE.L	A0,-(SP)	save for 'PRTSTG'
	BSR.W	TSTV		must be a variable now
	BCS.W	QWHAT		"What?" it isn't?
	MOVE.L	D0,A2		put away the variable's address
	MOVE.B	(A0),D2 	get ready for 'PRTSTG'
	CLR.B	D0
	MOVE.B	D0,(A0)
	MOVE.L	(SP)+,A1
	BSR.W	PRTSTG		print string as prompt
...
IP3	MOVE.L	A0,-(SP)	save in case of error
	...
	BSR.W	GETLN		then get an input line
	LEA	BUFFER,A0	point to the buffer
	BSR.W	EXPR		evaluate the input
...
IP4	ADDQ.L	#4,SP		clean up the stack
	BSR.W	TSTC		is the next thing a comma?
	DC.B	',',IP5-*
...
LET	BSR.W	SETVAL		do the assignment
	BSR.W	TSTC		check for more 'LET' items
	DC.B	',',LT1-*
	BRA	LET
...
POKE	BSR	EXPR		get the memory address
	BSR.W	TSTC		it must be followed by a comma
	DC.B	',',PKER-*
...
EXPR2	BSR.W	TSTC		negative sign?
	DC.B	'-',XP21-*
	CLR.L	D0		yes, fake '0-'
	BRA	XP26
XP21	BSR.W	TSTC		positive sign? ignore it
	DC.B	'+',XP22-*
...
XP24	MOVE.L	(SP)+,D1
	...
	BVS.W	QHOW		branch if there's an overflow
	BRA	XP23		else go back for more operations
XP25	BSR.W	TSTC		subtract?
	DC.B	'-',XP42-*
...
EXP4RT	RTS
XP41	BSR.W	TSTNUM		or is it a number?
	MOVE.L	D1,D0
	TST	D2		(if not, # of digits will be zero)
	BNE	EXP4RT		if so, return it in D0
PARN	BSR.W	TSTC		else look for ( EXPR )
	DC.B	'(',XP43-*
	BSR	EXPR
	BSR.W	TSTC
	DC.B	')',XP43-*
XP42	RTS
XP43	BRA.W	QWHAT		else say "What?"
...
TSTV	BSR.W	IGNBLK
	...
	ADD.L	D0,D0
	BCS.W	QHOW		say "How?" if index is too big
	ADD.L	D0,D0
	BCS.W	QHOW
	MOVE.L	D0,-(SP)	save the index
	BSR.W	SIZE		get amount of free memory
	MOVE.L	(SP)+,D1	get back the index
	CMP.L	D1,D0		see if there's enough memory
	BLS.W	QSORRY		if not, say "Sorry"
...
MLT2	CMP.L	#$FFFF,D1	is second argument <= 16 bits?
	...
	BHI.W	QHOW		one of them MUST be 16 bits
...
*** Rick Murray's bug correction follows:
	...
	BNE.W	QHOW		if overflow, say "How?"
	ADD.L	D2,D0		D0 now holds the product
	BMI.W	QHOW		if sign bit set, it's an overflow
...
LABS	BSR	PARN		get the following expr.'s value
	...
	BMI.W	QHOW		if still negative, it was too big
ABSRT	RTS
...
QWHAT	MOVE.L	A0,-(SP)
AWHAT	LEA	WHTMSG,A6
ERROR	BSR.W	PRMESG		display the error message
	MOVE.L	(SP)+,A0	restore the text pointer
	...
	BSR.W	PRTLN		display the line in error up to the 0
	MOVE.B	(SP)+,(A0)	restore the character
	MOVE.B	#'?',D0         display a "?"
	BSR	OUTCH
	CLR	D0
	SUBQ.L	#1,A1		point back to the error char.
	BSR.W	PRTSTG		display the rest of the line
...
*
GETLN	BSR	OUTCH		display the prompt
	MOVE.B	#' ',D0         and a space
	BSR	OUTCH
	LEA	BUFFER,A0	A0 is the buffer pointer
GL1	BSR.W	CHKIO		check keyboard
...
GL2	MOVE.B	D0,(A0)+	save the char.
	BSR	OUTCH		echo the char back out
...
QT2	ADDQ.L	#2,A1		skip 2 bytes on return
	JMP	(A1)		return
QT3	BSR.W	TSTC		is it a single quote?
	DC.B	'''',QT4-*
	MOVE.B	#'''',D0        if so, do same as above
	BRA	QT1
QT4	BSR.W	TSTC		is it an underline?
	DC.B	'_',QT5-*
...
*
CHKIO	BSR.W	INCH		get input if possible
	BEQ	CHKRET		if Zero, no input
	CMP.B	#CTRLC,D0	is it control-C?
	BNE	CHKRET		if not
	BRA.W	WSTART		if so, do a warm start
CHKRET	RTS
